...
(`hotato.conversation.v1`), each labelled `origin=simulated`. There is no live
agent, no TTS, and no network on this path -- it is the deterministic INPUT side
of a simulation: the ground-truth a caller holds, the caller's scripted turns,
...
(the produced transcript is content-hashed), so a run is a stable fixture, not a
sample.
...
> `hotato.scenario.v1` doc -- author one with `hotato simulate --init`. That is
> **not** the conversation-test file `hotato scenario init` writes (a
> `hotato.conversation-test.v1` that `hotato test run` consumes -- see
> [CONVERSATION-TEST.md](CONVERSATION-TEST.md)). If you feed a conversation-test
> file to `simulate` you get an actionable error that points you back to
> `--init`.

## What `--init` writes

`hotato simulate --init demo.scenario.json` writes a MINIMAL, valid
`hotato.scenario.v1` doc. It is a starter you edit for your own agent, never a
claim these are the right caller turns for your call. The scenario id is derived
...
The caller declares ONLY its own turns: a `say` is the caller speaking, and there
is no field for the agent's words. A scenario can never put words in the agent's
mouth -- that is enforced at the schema level, not hoped for at render time.
...
## The honesty invariants (structural, not documented-and-hoped)
...
- **Reproducibility is scoped to "a SEEDED REPLAY is byte-identical"**, never
  "the model is deterministic" -- there is no model here. A fixed
  `(scenario, seed)` produces the same transcript bytes every time; different
  seeds differ ONLY where the scenario allows it (probabilistic backchannels).
- **No `overall_score` / blended number anywhere** -- forbidden structurally, in
  both the schema and the code path.

## Reliability: pass@1 / pass@k / pass^k

`simulate` reports **Reliability** as its own dimension, never a blended score:
...
# assertions; SIMULATOR_INVALID runs are bucketed separately, never a PASS/FAIL:
hotato simulate --matrix demo.scenario.json \
    --conversation-test refund.test.yaml --parallel 8 --format json
```

The worker count never changes the byte-identical summary. Every result is
attributed per variation cell -- there is no blend and no `overall_score`.
...
| `1`  | at least one produced simulation was `SIMULATOR_INVALID` -- a broken fixture, never an agent PASS/FAIL -- or, under `--matrix --conversation-test`, a scored aggregate FAILed. |
