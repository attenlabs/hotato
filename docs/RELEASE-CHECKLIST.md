...
- [ ] **Re-mint the published Atlas records: `python3 scripts/regen_atlas_records.py`, then confirm with `python3 scripts/build_atlas.py --verify-reproduction`.** Every release re-mints them, because the packaged `__version__` is sealed into the failure record's provenance and the `record_id` is content-addressed over it -- so a version bump alone changes the id a reader reproduces. Run it with the PACKAGED hotato (a clean install of the wheel being shipped), not an editable checkout, since the script executes the displayed command chain in a fresh directory with whatever `hotato` is on PATH. Skipping this does NOT fail the tag build: it fails later, inside the publish workflow's full test suite, with `AtlasBuildError: atlas reproduction gate failed` -- after the tag is already pushed. It cost a failed 1.19.0 publish on 2026-07-31.
