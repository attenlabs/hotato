...
Conversation failed: Agent did not yield; measured talk-over was 2.66 s.
  ...
  Verify the record:  uvx --from hotato==1.19.0 hotato record verify hotato-failure-record/failure-record.json
```
...
Composite Action since v1.4.0. Adopt the current release (v1.19.0), pinned
by its full commit SHA; resolve the tag to its SHA first:

```bash
git ls-remote https://github.com/attenlabs/hotato refs/tags/v1.19.0
```
  ...
  conversation-qa:
    ...
    steps:
      ...
      - id: hotato
        ...
        uses: attenlabs/hotato@<full-commit-sha>  # v1.19.0
        with:
...
| an exact version, e.g. `1.19.0` | `pip install --no-deps hotato==1.19.0` |
