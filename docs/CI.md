...
Conversation failed: Agent did not yield; measured talk-over was 0.25 s.
  ...
  Verify the record:  uvx --from hotato==1.12.0 hotato record verify hotato-failure-record/failure-record.json
```
...
Composite Action since v1.4.0. Adopt the current release (v1.12.0), pinned
by its full commit SHA; resolve the tag to its SHA first:

```bash
git ls-remote https://github.com/attenlabs/hotato refs/tags/v1.12.0
```
  ...
  conversation-qa:
    ...
    steps:
      ...
      - id: hotato
        ...
        uses: attenlabs/hotato@<full-commit-sha>  # v1.12.0
        with:
...
| an exact version, e.g. `1.12.0` | `pip install --no-deps hotato==1.12.0` |
