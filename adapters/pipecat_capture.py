#!/usr/bin/env python3
"""Pipecat capture adapter for Hotato.

Capture your own Pipecat agent's turn-taking into a TWO-CHANNEL WAV
...
-------------------------
  * live capture -- inject a caller stimulus into your Pipecat pipeline and
    record the agent's TTS output, then write and score a two-channel WAV.
    Needs the optional ``pipecat`` stack, wired at the ADJUST points below.
...
Run the demo:
    ...
    PYTHONPATH=src python adapters/pipecat_capture.py --demo

Pipecat's transport and frame APIs move, so treat the capture body as the shape
of the integration, not a pinned snippet. Docs: https://docs.pipecat.ai
"""
...
# This one is a backchannel: the agent should HOLD the floor, not yield.
DEMO_SCENARIO = "02-backchannel-mhm"

_INSTALL_HINT = (
    "\nPipecat is not installed. The LIVE capture path needs the optional stack:\n"
    "    pip install pipecat-ai\n"
    "Then wire the three ADJUST points in capture_agent_response() to your\n"
    "pipeline. No agent yet? Run the zero-dependency demo instead:\n"
    "    PYTHONPATH=src python adapters/pipecat_capture.py --demo\n"
)
...
def capture_agent_response(
    ...
    """Play ``caller_wav_path`` into your Pipecat pipeline and return
    ``(sample_rate, agent_samples)`` recorded from the agent's TTS output.

    This is the only stack-specific part. The ``pipecat`` import is lazy so the
    ...
    caller_samples = caller.get(0)  # noqa: F841  (fed into ADJUST 1)

    async def _run() -> Tuple[int, List[float]]:
        # ADJUST 1: an input transport / frame source that plays `caller_samples`
        #           into the pipeline, delayed until `onset_sec` so the agent
        ...
        #           it and return the AGENT channel (the caller is your stimulus):
        #     import array
        #     agent_samples: List[float] = []
        #     @audiobuffer.event_handler("on_audio_data")
        #     async def _on_audio(buf, pcm, sr, num_channels):
        #         frames = array.array("h"); frames.frombytes(pcm)
        #         agent_samples.extend(x / 32768.0 for x in frames[1::2])
        #     await PipelineRunner().run(task)
        #     return sample_rate, agent_samples

        raise NotImplementedError(
            "Wire ADJUST 1-3 to your Pipecat pipeline, then return "
...
# The public `capture(...)` this adapter exposes: inject a caller stimulus into
# your pipeline, record the agent's TTS via a 2-channel AudioBufferProcessor, and
# write the two-channel WAV. Scaffold the drop-in recorder with:
...
def main(argv: Optional[List[str]] = None) -> int:
    parser = argparse.ArgumentParser(
        description="Pipecat capture -> hotato scorer (two-channel WAV)."
    )
    parser.add_argument(
        "--demo", action="store_true",
        help="zero-dependency: copy the bundled reference recording and score it",
    )
    parser.add_argument("--caller", help="caller stimulus WAV to inject into your pipeline (live capture)")
