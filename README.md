...
Your voice agent passes every text assertion and still loses the call. It talks over the caller, skips a required disclosure, or confirms a refund that never posted. hotato scores the call from the two-channel audio across five separate dimensions, shows the timing behind every flag, and turns each caught bug into a CI contract and a share-safe Failure Record. Paste that record into a PR and it seeds the next one.

## Catch a bug in one command
...
Offline, it sweeps two bundled demo calls, confirms one missed interruption, and writes a share-safe Failure Record to attach to a PR:
...
Feed the call as `--transcript`, `--trace`, `--state`, and/or `--audio`. A check with no evidence stays INCONCLUSIVE. Walkthrough: [`docs/CONVERSATION-TEST.md`](docs/CONVERSATION-TEST.md).

## Drive it from CI or an agent

hotato is built to be run by a coding agent, not just a human. Every command speaks a machine contract, so an agent scanning this repo can discover what hotato does and drive it in one fetch:

- **Machine JSON** &middot; `--format json` on any scoring command emits one stable envelope ([`envelope.v1.json`](https://hotato.dev/schema/envelope.v1.json)); every error shares one `error.v1` shape, never a raw traceback.
- **Meaningful exit codes** &middot; `0` pass &middot; `1` regression &middot; `2` refuse (a usage error, or a recording it cannot score). Gate CI on the exit code; do not parse prose.
- **MCP server** &middot; fifteen local tools over stdio: the `voice_eval_run` scorer, three counterexample tools, and eleven fleet tools. Audio stays on your machine.

  ```bash
  uvx --from "hotato[mcp]" hotato-mcp
  ```

- **Deterministic, content-addressed records** &middot; a Failure Record carries its own `sha256`; `hotato record verify` re-derives it from the same audio and config, so a record from a stranger's PR reproduces on your machine or it is not published.
- **One-fetch context** &middot; [`llms.txt`](llms.txt) indexes every command; [`llms-full.txt`](llms-full.txt) is the whole doc set in a single file; [`AGENTS.md`](AGENTS.md) and [`docs/MCP.md`](docs/MCP.md) carry the tool contract, the `hotato describe --format json` manifest, and copy-paste client configs (Claude Desktop, Cursor, Codex CLI).
...
<p align="center"><sub><b>Level 1: candidate.</b> A timing moment worth review, measured not judged.</sub></p>
...
### Five levels of evidence, one per dimension
...
- `hotato simulate --matrix scenario.yaml --out ./conv` &middot; expand a `scenario.v1` matrix into seeded, byte-identical runs.
- `hotato rubric run --rubrics rubrics.yaml --transcript call.json` &middot; the model-judged lane on a pinned local model, advisory unless `--gate`. [`docs/RUBRIC.md`](docs/RUBRIC.md).
- `hotato release compare BASELINE CANDIDATE` &middot; diff two recorded releases per dimension and scenario, digest-exact.
- `hotato serve` &middot; a read-only, token-authenticated web app over the local registry on `127.0.0.1`. [`docs/WORKSPACE.md`](docs/WORKSPACE.md).

The bundled reference agent runs a full offline, byte-reproducible battery across jobs, caller behaviours, and audio environments: `make reference` ([`examples/reference-agent/`](examples/reference-agent/README.md), [`docs/SUITE-RUN.md`](docs/SUITE-RUN.md)); measurement-error harness [`docs/BENCHMARK.md`](docs/BENCHMARK.md).
...
Your audio stays on your machine. Full guide: [`docs/SET-AND-FORGET.md`](docs/SET-AND-FORGET.md) &middot; [`examples/set-and-forget/`](examples/set-and-forget/README.md).
...
Counts, top candidate moments, artifact paths, and a Slack-ready `text` field. Egress: [`docs/EGRESS.md`](docs/EGRESS.md).
...
`hotato fleet trend -w acme` writes a self-contained per-agent trend page. Full guide: [`docs/GUARDIAN-FLEET.md`](docs/GUARDIAN-FLEET.md).
...
Air-gap, backup, a local judge, and one set of schemas for self-host and cloud: [`docs/SELF-HOST.md`](docs/SELF-HOST.md). Verify the offline posture yourself with [`deploy/verify-zero-egress.sh`](deploy/verify-zero-egress.sh).
...
A contract bundle contains call audio, so keep raw customer contracts out of public repos; use sanitized fixtures. See [`docs/CONTRACTS.md`](docs/CONTRACTS.md).
...
- **Recorded-call battery**: [`corpus/vapi-defaults/README.md`](corpus/vapi-defaults/README.md)
...
- **Checking today's agent, not just the frozen recording**: [`docs/RECAPTURE.md`](docs/RECAPTURE.md)
...
<p align="center">
  <img src=".github/assets/hotato-lockup.svg" alt="hotato" width="210" style="max-width:100%;height:auto;">
</p>

Why "hotato": good turn-taking is a game of hot potato. Hold the turn a beat too long and you have already lost it. Speak, then pass the turn the moment the caller wants it. MIT licensed ([`LICENSE`](LICENSE)); the open core stays open.
